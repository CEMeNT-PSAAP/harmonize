...
int main(int argc, char *argv[]){
	...
	// Number of cross section lookups per step
	dev_state.lookups_per_step  = args["lookups_per_step"];

	// Number of possible cross sections to lookup
	int cross_section_count = args["cross_count"];

	float fission_max = args["fx_max"];
	float scatter_max = args["sx_max"];
	float capture_max = args["cx_max"];
	...
	// Arrays of random cross sections
	host::DevBuf<float> fission;
	host::DevBuf<float> scatter;
	host::DevBuf<float> capture;

	std::vector<float> cpu_fission;
	std::vector<float> cpu_scatter;
	std::vector<float> cpu_capture;

	cpu_fission.resize(cross_section_count);
	cpu_scatter.resize(cross_section_count);
	cpu_capture.resize(cross_section_count);

	for (int i=0; i<cross_section_count; i++) {
		cpu_fission[i] = fission_max * (rand() % 1000000) / 1000000.0;
	...
	dev_state.cross_section_count = cross_section_count;

	dev_state.fission_x = fission;
	dev_state.scatter_x = scatter;
	dev_state.capture_x = capture;
