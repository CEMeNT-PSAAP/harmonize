...
struct Step {
	...
	__device__ static void eval(PROGRAM prog, Neutron arg){
		...
		for (int i=0; i<result; i++) {
			Neutron new_neutron = n.child(n.callback);
			unsigned int index = prog.group.cache.alloc_index(prog.thread.rand_state);
			(*prog.device.neutron_pool)[index] = new_neutron;
			n.callback = index;
		}
		...
		} else if (n.callback != ADR_NULL ) {
			Neutron callback_neutron = (*prog.device.neutron_pool)[n.callback];
			prog.template async<Step>(callback_neutron);
			prog.group.cache.free(n.callback,prog.thread.rand_state);
		}
...
struct MyProgramSpec {
	...
	static const size_t STASH_SIZE = 5;
	...
	__device__ static void initialize(PROGRAM prog) {
		...
		prog.group.cache.initialize(*prog.device.neutron_pool);
	}
...
int main(int argc, char *argv[]){
	...
	unsigned int dyn_pool_size = args["dyn_pool"] | 0x800000;
	...
	// Set up a pool to dynamicall allocate neutrons from
	host::DevObj<mem::MemPool<Neutron,unsigned int>> neutron_pool = host::DevObj<mem::MemPool<Neutron,unsigned int>>(dyn_pool_size,8191u);
	dev_state.neutron_pool = neutron_pool;
	...
	if( !dev_state.is_async ){
		...
		do {
			...
			exec<AsyncProgram>(instance,wg_count,0x100000);
			cudaDeviceSynchronize();
			check_error();
			num++;
		} while(! instance.complete() );
