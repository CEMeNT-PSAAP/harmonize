...
__device__ void lookup_busywork(PROGRAM prog, Neutron& arg) {

	unsigned int index;
       
	for(int i=0; i<30; i++) {
		index = random_uint(arg.seed) % 65536;
		arg.seed |= atomicAnd(&prog.device.dummy_data[index],1);
	}

}


// Advances neutrons forward in the simulation
struct Step {
	...
	__device__ static void eval(PROGRAM prog, Neutron arg) {
		...
		if( result == 0 ) {
			bool needs_lookup = (random_uint(n.seed) % 8191) < 512;

			bool do_step = true;
			if ( needs_lookup ) {
				if ( prog.device.separate_lookup ) {
					prog. template async<Lookup>(n);
					do_step = false;
				} else {
					lookup_busywork(prog,n);
				}
			}
			if( do_step ) {
...
// Emulates the memory fetching cost of an uncached cross section lookup.
struct Lookup {
...
struct MyProgramSpec {
	...
	typedef OpUnion<Step,Lookup> OpSet;
...
using AsyncProg = HarmonizeProgram < MyProgramSpec >;
...
int main(int argc, char *argv[]){
	...
	dev_state.separate_lookup   = args["lookup"];
	...
	// Dummy lookup data
	host::DevBuf<unsigned int> dummy_data;
	dummy_data.resize(65536);
	dev_state.dummy_data = dummy_data;
	cudaMemset( dummy_data, 0, sizeof(unsigned int) * 65536 );
