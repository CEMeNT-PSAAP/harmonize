...
try:
    ...
    print("failed to load numba.cuda")
    print()
    print(
    ...
    if "return" in func.__annotations__:
        ...
        if res_ano != res_type:
            ...
            err_str = (
                ...
                + func.__name__
                + "' results in a return type  matching its "
...
def global_ptx(func):
    ptx, res_type = cuda.compile_ptx_for_current_device(
        func, fn_arg_ano(func), debug=DEBUG, opt=(not DEBUG)
    )
...
def device_ptx(func):
    # print(func.__name__)
    ptx, res_type = cuda.compile_ptx_for_current_device(
        func, fn_arg_ano(func), device=True, debug=DEBUG, opt=(not DEBUG)
    )
    assert_fn_res_ano(func, res_type)
    return ptx, res_type
...
def map_type_name(type_map, kind, rec_mode=""):
    ...
    if kind in primitives:
        ...
        raise RuntimeError(
            "Unrecognized type '" + str(kind) + "' with type '" + str(type(kind)) + "'"
        )
...
def func_param_text(func, type_map, rec_mode="", prior=False, clip=0, first_void=False):
    ...
    if first_void:
        ...
        param_list = ["void*"] + [
            map_type_name(type_map, kind, rec_mode=rec_mode) for kind in param_list
        ]
    else:
        param_list = [
            map_type_name(type_map, kind, rec_mode=rec_mode) for kind in param_list
...
def harm_template_func(
    func, template_name, function_map, type_map, inline, suffix, base=False
):
...
class AsyncRuntime(Runtime):
    ...
    def exec(self, cycle_count, wg_count=1):
        self.fn["exec_program"](
            self.context_ptr, self.state_ptr, cycle_count, wg_count, 32
        )
...
class RuntimeSpec:
    ...
    def generate_meta(self):
        ...
        self.meta["PROMISE_ARR_TYPE"] = np.dtype(
            (self.meta["UNION_TYPE"], self.meta["GROUP_SIZE"])
        )
        ...
        self.meta["IO_BUFFER_ARRAY_TYPE"] = np.dtype((np.uintp, (len(self.async_fns),)))
        ...
        self.meta["WORK_POOL_ARR_TYPE"] = np.dtype(
            (self.meta["QUEUE_TYPE"], (self.meta["QUEUE_COUNT"],))
        )
        ...
        self.meta["FRAME_ARR_TYPE"] = np.dtype(
            (self.meta["WORK_FRAME_TYPE"], (self.meta["STACK_SIZE"],))
        )
    ...
    def generate_specification_code(
        ...
        for func in self.async_fns:
            ...
            return_type = "void"
            if "return" in func.__annotations__:
                return_type = map_type_name(
                    self.type_map, func.__annotations__["return"]
                )
            proto_decls += (
                f'extern "C" __device__ int _{func.__name__}_{suffix}'
        ...
        for func in self.async_fns:
            async_defs += harm_async_func(
                func, self.function_map, self.type_map, inline, suffix
            )
        ...
        meta_defs = "".join(
            [
                "\tstatic const size_t " + name + " = " + str(value) + ";\n"
        ...
        union_def = (
            "\ttypedef OpUnion<"
            + ",".join([pascal_case(str(func.__name__)) for func in self.async_fns])
            + "> OpSet;\n"
        )
        ...
        state_defs = (
            "\ttypedef "
            + map_type_name(self.type_map, self.dev_state, rec_mode="ptr")
            + " DeviceState;\n"
            + "\ttypedef "
            + map_type_name(self.type_map, self.grp_state, rec_mode="ptr")
            + " GroupState;\n"
            + "\ttypedef "
            + map_type_name(self.type_map, self.thd_state, rec_mode="ptr")
            + " ThreadState;\n"
        )
    ...
    def generate_specialization_code(self, kind, shorthand, suffix):
        ...
        preamble += spec_decl_template.format(
            kind=kind, name=self.spec_name, short_name=short_name, suffix=suffix
        )

        # Generate the wrappers for kernel entry points
        dispatch_defs += init_template.format(
            short_name=short_name, name=self.spec_name, kind=kind, suffix=suffix
        )
        dispatch_defs += exec_template.format(
            short_name=short_name, name=self.spec_name, kind=kind, suffix=suffix
        )

        if kind == "Event":
            dispatch_defs += alloc_event_prog_template.format(
                short_name=short_name, suffix=suffix
            )
        else:
            dispatch_defs += alloc_harm_prog_template.format(
                short_name=short_name, suffix=suffix
            )

        dispatch_defs += free_prog_template.format(short_name=short_name, suffix=suffix)
        dispatch_defs += alloc_state_template.format(
            state_struct=state_struct, suffix=suffix
        )
        ...
        dispatch_defs += clear_flags_template.format(
            short_name=short_name, suffix=suffix
        )
        ...
        for fn in self.async_fns:
            ...
            arg_text = func_arg_text(
                fn, self.type_map, rec_mode="cast_deref", prior=False, clip=1
            )
        ...
        for field, prefix in program_fields:
            accessor_defs += accessor_template.format(
                short_name=short_name, field=field, prefix=prefix, suffix=suffix
            )
        ...
        if False:
            ...
            for field, kind, prefix in query_list:
                query_defs += query_template.format(
                    short_name=short_name, field=field, kind=kind, prefix=prefix
                )
    ...
    def bind_and_load():
        ...
        for name, spec in RuntimeSpec.registry.items():
            for kind, shortname in RuntimeSpec.kinds:
                ...
                exec_program = ext_fn(
                    f"exec_program_{suffix}", sig(void, vp, usize, usize)
                )
                ...
                if kind == "Event":
                    # IO_SIZE, LOAD_MARGIN
                    alloc_program = ext_fn(
                        f"alloc_program_{suffix}", sig(vp, vp, usize)
                    )
