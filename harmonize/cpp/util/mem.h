...
struct MemPool {
	...
	{
		if( arena_size.adr != 0 ){
			host::auto_throw( adapt::rtFree( arena ) );
		}
		if( pool_size.adr  != 0 ){
			host::auto_throw( adapt::rtFree( pool  ) );
		}
	}
	...
	__host__ void next_print(){
		...
		adapt::rtMemcpy(host_copy,arena,sizeof(Link)*arena_size.adr,adapt::rtMemcpyDeviceToHost);
