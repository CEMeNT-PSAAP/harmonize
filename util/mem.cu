...
struct Adr
{
	...
	Adr<AdrType>() = default;
	 ...
	 __device__ Adr<AdrType>(AdrType adr_val) : adr(adr_val) {}

	 __device__ bool operator==(const Adr<AdrType>& link_adr){
...
struct PoolQueue <Adr<ADR_TYPE>>
{
	...
	*/
	 __device__ PoolQueue<Adr<ADR_TYPE>>
	...
	*/
	...
	__device__ static const Self null_queue() {
		Self result;
...
struct MemPool {
	...
	{
		arena  = hardMalloc<Link>     ( arena_size.adr );
		pool   = hardMalloc<QueueType>( pool_size .adr );
